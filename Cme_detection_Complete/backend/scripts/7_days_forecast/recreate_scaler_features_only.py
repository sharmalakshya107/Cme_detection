...
# Get ONLY feature columns (exclude targets)
all_cols = [col for col in train_data.columns if col not in exclude_cols]
feature_cols = [col for col in all_cols if col not in target_vars]

print(f"Total columns in data: {len(all_cols)}")
print(f"Target columns: {target_vars}")
print(f"Feature columns (to scale): {len(feature_cols)}")
print(f"Features: {feature_cols}")

# Prepare data for scaling (ONLY features, no targets)
X = train_data[feature_cols].fillna(0).values
print(f"\nData shape: {X.shape}")

# Fit scaler on features only
print("\nFitting StandardScaler on features only...")
...
print("\n" + "="*70)
print("✅ SCALER RECREATED SUCCESSFULLY (FEATURES ONLY)")
print("="*70)
